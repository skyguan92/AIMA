...
int main(int argc, const char *argv[]) {
  ...
  std::string a_bin = vm["a_bin"].as<std::string>();
  std::string b_bin = vm["b_bin"].as<std::string>();
  std::string c_bin = vm["c_bin"].as<std::string>();
  ...
  if (!a_bin.empty()) {
    std::ifstream in(a_bin, std::ios::binary);
    if (!in) {
      std::cerr << "Failed to open a_bin file: " << a_bin << std::endl;
      return 1;
    }
    in.read(reinterpret_cast<char *>(AVec.data()),
            (std::streamsize)(AVec.size() * sizeof(A_DATATYPE)));
    if (!in || in.gcount() != (std::streamsize)(AVec.size() * sizeof(A_DATATYPE))) {
      std::cerr << "Failed to read full A buffer from: " << a_bin << std::endl;
      return 1;
    }
  } else {
  ...
  if (!b_bin.empty()) {
    std::ifstream in(b_bin, std::ios::binary);
    if (!in) {
      std::cerr << "Failed to open b_bin file: " << b_bin << std::endl;
      return 1;
    }
    in.read(reinterpret_cast<char *>(BVec.data()),
            (std::streamsize)(BVec.size() * sizeof(B_DATATYPE)));
    if (!in || in.gcount() != (std::streamsize)(BVec.size() * sizeof(B_DATATYPE))) {
      std::cerr << "Failed to read full B buffer from: " << b_bin << std::endl;
      return 1;
    }
  } else {
  ...
  if (!c_bin.empty()) {
    memcpy(CVec.data(), bufOut, (CVec.size() * sizeof(C_DATATYPE)));
    std::ofstream out(c_bin, std::ios::binary);
    if (!out) {
      std::cerr << "Failed to open c_bin for write: " << c_bin << std::endl;
      return 1;
    }
    out.write(reinterpret_cast<const char *>(CVec.data()),
              (std::streamsize)(CVec.size() * sizeof(C_DATATYPE)));
    if (!out) {
      std::cerr << "Failed to write C buffer to: " << c_bin << std::endl;
      return 1;
    }
  }
