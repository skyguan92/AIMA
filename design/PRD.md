...
```
  ...
  │     aima up --engine vllm --gpu-mem 0.5      │
  ├─────────────────────────────────────────────┤
...
| Agent 不可用 | — | `aima up <model>` 手动部署 | L1 |
| 全新硬件无知识 | — | `aima up --engine vllm --config '{...}'` | L1→L0 |
...
| ID | 需求 | 优先级 | 版本 |
|----|------|--------|------|
| D1 | `aima up <model>` 一条命令部署 (自动匹配引擎+配置) | P0 | MVP |
...
| 知识库冷启动，初期 Recipe 覆盖低 | 新用户体验差 | 高 | 内嵌主流组合 + L0 保守默认值 + 首批用户定向支持 |
