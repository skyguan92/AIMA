...
当前共 94 个 MCP 工具覆盖所有功能领域 (Hardware 2 + Model 6 + Engine 7 + Deploy 8 + Knowledge 23 + Benchmark 4 + Stack 3 + Catalog 3 + System 2 + Discover 1 + Agent 9 + Fleet 4 + Shell 1 + Tuning 4 + Device 2 + App 3 + OpenClaw 3 + Scenario 3 + Explore 4 + Support 1 + Download 1)。
...
- **Fleet 工具拦截**: `fleet.exec_tool` 在远程执行时屏蔽破坏性工具 (`model.remove`, `engine.remove`, `deploy.delete`, `stack.init`, `agent.rollback`, `shell.exec`)，防止 Agent 通过远程 Fleet 调用绕过本地安全护栏。
...
*最后更新：2026-04-01 (v0.2.0: 94 MCP tools, OpenClaw, Scenarios, Explore, Support, Central Server, TUI)*
