...
当前只有 `agent.ask` 通过 `ListToolsForProfile()` 消费 profile；Explorer 的 LLM planner 仍是纯文本规划，不依赖工具列表。
...
*最后更新：2026-04-08 (v0.2.0 branch consolidation: 56 MCP tools, action-based fleet/openclaw/stack/explore surfaces, profile-filtered agent.ask only)*
