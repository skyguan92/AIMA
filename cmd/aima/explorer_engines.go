...
// modelMaxContextLen extracts the largest max_model_len or context_length
// from a model's catalog variants. Returns 0 if unknown.
func modelMaxContextLen(cat *knowledge.Catalog, modelName string) int {
	if cat == nil {
		return 0
	}
	for i := range cat.ModelAssets {
		if !strings.EqualFold(cat.ModelAssets[i].Metadata.Name, modelName) {
			continue
		}
		var best int
		for _, v := range cat.ModelAssets[i].Variants {
