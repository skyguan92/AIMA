...
OUT = Path("/tmp/qwen3.6-gb10-dflash-round12-combo-business.json")
...
CASES = [
    {
        "label": "business_maxseqs1_rerun",
        "extra_args": [],
    },
    {
        "label": "business_maxseqs1_no_reserve",
        "extra_args": ["--no-scheduler-reserve-full-isl"],
    },
    {
        "label": "business_maxseqs1_no_async",
        "extra_args": ["--no-async-scheduling"],
    },
    {
        "label": "business_maxseqs1_no_reserve_no_async",
        "extra_args": ["--no-scheduler-reserve-full-isl", "--no-async-scheduling"],
    },
]
...
def bench(case_label, inp, prompt):
    res = run(
        [
            ...
            prompt,
            "--notes",
            f"round12 {case_label} {inp//1024}k 1k business",
        ],
        timeout=2600,
    )
