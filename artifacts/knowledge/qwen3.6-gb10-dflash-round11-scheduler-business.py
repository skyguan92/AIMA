...
OUT = Path("/tmp/qwen3.6-gb10-dflash-round11-scheduler-business.json")
...
CASES = [
    {
        "label": "business_best_rerun",
        "extra_args": [],
    },
    {
        "label": "business_best_maxseqs1",
        "extra_args": ["--max-num-seqs", "1"],
    },
    {
        "label": "business_best_partial2_long1_auto",
        "extra_args": [
    ...
    {
        "label": "business_best_no_reserve_full_isl",
        "extra_args": ["--no-scheduler-reserve-full-isl"],
    },
    {
        "label": "business_best_no_async_scheduling",
...
def bench(case_label, inp, prompt):
    res = run(
        [
            ...
            prompt,
            "--notes",
            f"round11 {case_label} {inp//1024}k 1k business",
        ],
        timeout=2600,
    )
