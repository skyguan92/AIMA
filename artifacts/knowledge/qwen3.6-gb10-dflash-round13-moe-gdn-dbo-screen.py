...
OUT = Path("/tmp/qwen3.6-gb10-dflash-round13d-moe-screen.json")
...
CASES = [
    {
        "label": "moe_flashinfer_trtllm",
        "extra_args": ["--moe-backend", "flashinfer_trtllm"],
        "extra_env": [],
    },
    {
        "label": "moe_ep_flashinfer_cutlass",
        "extra_args": ["--enable-expert-parallel", "--moe-backend", "flashinfer_cutlass"],
        "extra_env": [],
    },
]
...
def bench(case_label, prompt):
    res = run(
        [
            ...
            prompt,
            "--notes",
            f"round13 {case_label} 64k 1k business screen",
        ],
        timeout=2800,
    )
