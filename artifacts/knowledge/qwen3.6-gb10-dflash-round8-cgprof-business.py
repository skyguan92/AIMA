...
OUT = Path("/tmp/qwen3.6-gb10-dflash-round8-cgprof-business.json")

CASES = [
    {
        "label": "business_noneager_cgprof_gmu092",
        "gpu_memory_utilization": "0.92",
        "extra_env": [("VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS", "1")],
    },
    {
        "label": "business_noneager_cgprof_gmu09435",
        "gpu_memory_utilization": "0.9435",
        "extra_env": [("VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS", "1")],
    },
]
...
def bench(case_label, inp, prompt):
    res = run(
        [
            ...
            prompt,
            "--notes",
            f"round8 {case_label} {inp//1024}k 1k business",
        ],
        timeout=2600,
    )
