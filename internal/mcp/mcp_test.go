...
func TestProfileFilteringInServer(t *testing.T) {
	s := NewServer()
	...
	defs = s.ListTools()
	operatorExpected := map[string]bool{
		"hardware.detect": true, "hardware.metrics": true,
		"model.list": true, "model.scan": true,
		"deploy.apply": true, "deploy.list": true,
		"knowledge.resolve": true,
		"agent.ask": true,
	}
	if len(defs) != len(operatorExpected) {
		t.Errorf("operator profile: got %d tools, want %d", len(defs), len(operatorExpected))
	}
	for _, d := range defs {
		if !operatorExpected[d.Name] {
			t.Errorf("operator profile: unexpected tool %q", d.Name)
		}
	}
	...
	defs = s.ListTools()
	patrolExpected := map[string]bool{
		"deploy.apply": true, "deploy.list": true,
		"knowledge.resolve": true,
		"benchmark.run":        true,
		"agent.patrol_status":  true,
		"hardware.metrics":     true,
	}
	if len(defs) != len(patrolExpected) {
		t.Errorf("patrol profile: got %d tools, want %d", len(defs), len(patrolExpected))
	}
...
func TestExecuteToolIgnoresProfile(t *testing.T) {
	...
	// ListTools should NOT include it
	defs := s.ListTools()
	for _, d := range defs {
		if d.Name == "knowledge.gaps" {
			t.Error("knowledge.gaps should be hidden by operator profile")
		}
	}
